...
KON_ROOT = "/home/k303/object-detection/dataset/KON/"
...
class KONAnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None):
        self.class_to_ind = class_to_ind or dict(
            zip(KON_CLASSES, range(len(KON_CLASSES))))
...
class KONDetection(data.Dataset):
    """KON Detection Dataset Object
    ...
    def __init__(self, root, img_size=None,
                 ...
                 transform=None, 
                 base_transform=None,
                 target_transform=KONAnnotationTransform(),
                 dataset_name='kon', mosaic=False):
        self.root = root
    ...
    def load_data_indexs(self):
        ...
        for filename in filename_list:
            ...
            self.target_names_list.append(filename.split('.')[0])
...
if __name__ == "__main__":
    ...
    size = 640
    dataset = KONDetection(root=KON_ROOT, img_size=size,
                            transform=BaseTransform([size, size], (0, 0, 0)),
                            base_transform=BaseTransform([size, size], (0, 0, 0)),
                            mosaic=True)

    cls_color = [(255, 0, 255), (18, 153, 255), (255, 0, 0), (0, 255, 0), (203, 192, 255)]
    ...
    for i in range(0, len(dataset)):
        ...
        for box in gt:
            xmin, ymin, xmax, ymax, cls_ind = box
            xmin *= size
            ymin *= size
            xmax *= size
            ymax *= size
            img = cv2.rectangle(img, (int(xmin), int(ymin)), (int(xmax), int(ymax)), cls_color[int(cls_ind)], 2)
        cv2.imshow('gt', img)
