...
class MAPEvaluator(object):
    ...
    """
    def __init__(self, device, dataset=None, classname=[], ovthresh=0.5, name='kon', display=False):
        self.device = device
        ...
        self.output_dir = self.get_output_dir('ap_eval/', 'test')
    ...
    def evaluate(self, model, use_07_metric=True):
        ...
        for i in range(self.num_datas):
            im, _, h, w, offset, scale = self.dataset.pull_item(i)
...
if __name__ == "__main__":
    ...
    from data.kon import KONDetection, KON_ROOT, KON_CLASSES
    from data import BaseTransform
    from utils.augmentations import SSDAugmentation
    import os

    dataset_root = '/home/k303/object-detection/dataset/kon'

    # we use our ship dataset
    data_dir = KON_ROOT
    input_size = [640, 640]
    dataset = KONDetection(root=data_dir, 
    ...
    # build a model
    model = CenterFace('cuda', input_size=input_size, num_classes=5)
    model.to('cuda')

    # load net
    model.load_state_dict(torch.load('/home/jxk/object-detection/KonFace/weights/centerface/centerface.pth', map_location='cuda'))
    model.eval()

    val_evaluator = MAPEvaluator(device='cuda', 
                                 dataset=dataset,
    ...
    val_evaluator.evaluate(model=model)
